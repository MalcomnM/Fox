...
# This is to help us find lines tagged with a Fox
...
# -% Globals %-
#
# FOX[dict]: Results from each line trace
FOX = []
COUNTER = 1
#########
...
def fox_prints():
    ...
    results = (json.dumps(i) for i in FOX if contains_any(
        'value', i.keys()) or i['error'])
    python_data = ", ".join(results)

    # DO NOT TOUCH, ie: no pretty printing
    print("WOOF: [" + python_data + "]")  # <--  Fox result
    ######################################
...
def result_handler(event):
    """
        ...
        Side Effects: Results are appended to the global FOX list.
    """
    ...
    # # how it works.
    match = FOX_MACROS.search(source)
    ...
    if match:
        ...
        if match.group('print'):
            buffer = io.StringIO()
            try:
                to_eval = "print({}, file=fox__buffer__)".format(
                    match.group('print'))
                exec(to_eval, _globals, {**_locals, 'fox__buffer__': buffer})
        ...
        if not skip and event.kind not in ['return', 'call']:
            # And lastly, update our FOX results list
            FOX.append(metadata)
...
def main(filename):
    """
        ...
        are stored in the global FOX variable which are
        ...
        On success:
            ...
            -> `WOOF:` a string search for this tag returns the
                ...
                Ex:
                    ...
                    $ python fox.py /some/path/to/script.py
                    ...
                    WOOF: [{...}, {...}, ...]
        ...
        `fox.py Debug Session` from the list to start debugging with the
